...
const simulatorChecks = [
  ...
  ["completes an instant run across the full service", /action === "instant_run"[\s\S]+advance\(url, headers, venue\.id, venue\.slug, state, state\.speed, true\)[\s\S]+simulationTargetMinute\(progress\.minute, SERVICE_MINUTES, completeImmediately\)/],
  ["paces quick-start progress through the shared simulation clock", /simulationProgress\(state\.simulated_minute[\s\S]+last_tick_at: completeImmediately \? tickedAt\.toISOString\(\) : progress\.lastTickAt/],
